...
def generate_vertex(r_range=(0, 64), z_range=(-142, -6), size=1):
...
def single_s1_instruction(interaction_type, energy, N_events=1):
    ...
    if (type(energy) != float) and (type(energy) != int):
        assert (
            len(energy) == 2
        ), "You must input a single energy or array like [low, high] for uniform dist"
    ...
    for i in range(0, N_events):
        ...
        yields = nc.GetYields(
            nestpy.INTERACTION_TYPE(interaction_type), e, density, instr["local_field"]
        )
...
def instruction(interaction_type, energy, N=10000):
    ...
    # If Krypton, simulate the whole event using generators.
        ...
        fax_instr = generator.generator_Kr83m(
            n_tot=N,
            recoil=11,
            rate=30.0,  # in Hz, hardcoded in case
            fmap=FIELD_MAP,
...
def instr_file_name(fax_instr, interaction_type, energy):
    ...
    if (type(energy) == float) or (type(energy) == int):
        file_name = (
            "/dali/lgrandi/yuanlq/s1_wf_comparison/wfsim_config/int%s_e%s_%s_%s.csv"
            % (interaction_type, int(energy), int(energy), now_str)
        )
    else:
        file_name = (
            "/dali/lgrandi/yuanlq/s1_wf_comparison/wfsim_config/int%s_e%s_%s_%s.csv"
            % (interaction_type, int(energy[0]), int(energy[1]), now_str)
        )
...
def sim_peak_extra(peaks, peak_basics, truth, match):
    ...
    dtypes = [
        ...
        (("Integral per channel [PE]", "area_per_channel"), np.float32, 494),
        ...
        (
            ("Number of hits contributing at least one sample to the peak", "n_hits"),
            np.int32,
        ),
        (
            ("PMT channel within tight range of mean", "tight_coincidence_channel"),
            np.int16,
        ),
        (("Classification of the peak(let)", "type"), np.int8),
        (
            ("Waveform data in PE/sample (not PE/ns!)", "data"),
            np.float32,
            NSUMWVSAMPLES,
        ),
        (
        ...
        (
            ("Peak widths in range of central area fraction [ns]", "width"),
            np.float32,
            NWIDTHS,
        ),
    ...
    for i in range(len(dtypes)):
        ...
        if (
            (field == "data")
            or (field == "area_decile_from_midpoint")
            or (field == "n_hits")
            or (field == "area_per_channel")
            or (field == "width")
            or (field == "max_gap")
            or (field == "max_goodness_of_split")
        ):
