...
def get_config(config_file="fax_config_nt_sr0_dev.json"):
    """Get wfsim configuration file.

    Args:
        config_file (str, optional): Configuration file name. Defaults to 'fax_config_nt_sr0_dev.json'.
    """
    config = straxen.get_resource(config_file, config_file.split(".")[-1])
    return config


def scintillation(interaction_type, e_dep, config_file="fax_config_nt_sr0_dev.json"):
    ...
    Args:
        ...
        config_file (str, optional): File as wfsim config. Defaults to 'fax_config_nt_sr0_dev.json'.
    ...
    hist = Hist1d(
        [],
        bins=np.linspace(-100, 1000, 1101),
    )
    for i in range(10000):
        energy_deposit = e_dep

        y = nestpy_calc.GetYields(
            interaction,
            energy_deposit,
            lxe_density,
            local_field,
        ...
        scint_time = np.clip(
            scint_time, 0, config.get("maximum_recombination_time", 10000)
        )
...
def propagation(
    ...
    prop_t, prop_y_top = optical_propagation(
        top=True, z_position=z_position, spline_file=spline_file
    )
    prop_t, prop_y_bot = optical_propagation(
        top=False, z_position=z_position, spline_file=spline_file
    )
...
def optical_propagation(
    top, z_position, spline_file="XENONnT_s1_proponly_va43fa9b_wires_20200625.json.gz"
):
...
def transit(config_file="fax_config_nt_sr0_dev.json"):
    ...
    tts_y = gaussian(
        ...
        config["pmt_transit_time_spread"] / 2.355,
    )
    tts_y = tts_y / tts_y.sum()
    return tts_t, tts_y


def spe(config_file="fax_config_nt_sr0_dev.json"):
    """SPE pulse shape.
...
def sims1(
    ...
    spline_file="XENONnT_s1_proponly_va43fa9b_wires_20200625.json.gz",
    pattern_map_file="XENONnT_s1_xyz_patterns_LCE_corrected_qes_MCva43fa9b_wires.pkl",
    config_file="fax_config_nt_sr0_dev.json",
):
    ...
    Args:
        ...
        config_file (str, optional): Configuration file name. Defaults to 'fax_config_nt_sr0_dev.json'.
    """
    prop_t, prop_y = propagation(
        z_position=z_position,
        spline_file=spline_file,
        pattern_map_file=pattern_map_file,
    )
    scint_t, scint_y = scintillation(
        interaction_type=interaction_type, e_dep=e_dep, config_file=config_file
    )
...
def get_s1_templates(
    interaction_type,
    e_dep,
    z_positions=ZSLIACES,
    ...
    Args:
        ...
        pattern_map_file (str): path to map. eg. '/dali/lgrandi/xenonnt/simulations/optphot/mc_v4.1.0/S1_1.63_0.99_0.99_0.99_0.99_5500_10000_30/XENONnT_S1_xyz_patterns_LCE_corrected_QEs_MCv4.1.0_wires.pkl'
        config_file (str, optional): [description]. Defaults to 'fax_config_nt_sr0_dev.json'.
    ...
    if plot:
        ...
        plt.xlabel("time [ns]")
        plt.title(
            "WFSIM S1 at different positions [E=%skeV, NESTtype%s]"
            % (e_dep, interaction_type)
        )
        plt.xlim(100, 500)
        plt.show()
