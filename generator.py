...
def generator_Ar37(n_tot=1000, rate=20., fmap=None, nc=None, 
...
def generator_flat(en_range=(0,30.0),
                   recoil=7, 
...
def generator_Kr83m(
                    ...
                    mode="all", 
                    timemode="realistic",filterzero=True):
    """
    ...
    if recoil==11:
        if version.parse(nestpy.__version__)<version.parse("1.5.4"):
            print("WARNING! Using nestpy before 1.5.4, NEST yields for 9 and 41 keV lines are not reliable!")
        for i in range(0, n_tot):
            assert(instr['type'][4*i]==1)
            assert(instr['type'][4*i+1]==2)
            y_= nc.GetYieldKr83m(energy=instr['e_dep'][4*i], 
            ...
            y_= nc.GetYieldKr83m(energy=instr['e_dep'][4*i+2], 
